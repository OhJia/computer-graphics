...
CT.ShapePath = function() {
   this._draw = function() {
      ...
      if (isFill) {
         ...
         gl.uniform1f (this._address('uNpts'   ), path.length / 3);
         gl.uniform3fv(this._address('uPath'   ), path);
      ...
      this._drawArrays(2 * (path.length / 3 + (isFill ? 1 : 10)));
   };
   ...
   this._defaultVertexShader =
      ...
      ,'   }'
      ,'   gl_Position = uCamMatrix * vec4(p0 + vec3(radius * dp, 0.), 1.);'
      ,'}'
      ].join('\n');
